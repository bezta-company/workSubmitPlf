...
if __name__ == "__main__":
    class Kmeans:
        ...
        def getCurCenter(self, loop, data,headers,clusterNum):
            if loop < 0:
        ...
        def gernerateNewCluster(self, sample, center_points,headers):
        ...
        def process(self,maxLoop,threshold):
            last_dist = 0
            try:
                #读取初始待聚类的数据
                clusterDt = Kmeans.getClusterData()
                for loop in range(0, maxLoop):
                    if loop % 5 == 0:
                        print('Loop times:%d' % loop)
                    #计算当前迭代中的簇中心点
                    curCenter = Kmeans.getCurCenter(loop, clusterDt)
                    #计算当前迭代中每条样本所归属的簇标签
