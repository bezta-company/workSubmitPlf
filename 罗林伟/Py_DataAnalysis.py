...
lianjia_df = pd.read_csv(r"F:\微信下载文件\WeChat Files\wxid_j2ddsekq7wx422\FileStorage\File\2019-09\lianjia.csv")
# Region     -- 区县名称
# District   -- 地区
# Garden     -- 小区
# Layout     -- 户型
...
# house_c = df.groupby(by=['Layout', 'Year'])['Year'].count().sort_values(ascending=False).to_frame()
# print(house_c)
# 不同Year下的房屋总价的均值对比(可绘图，也可以用数字呈现)
# house_m = df.groupby(by='Year')['Price'].mean().sort_values(ascending=False).to_frame()
# print(house_m)
# 不同Year下的房屋总面积均值对比
# house_m1 = df.groupby(by='Year')['Size'].mean().sort_values(ascending=False).to_frame()
# print(house_m1)
# 不同Year下的带和不带花园的房屋总数对比
# house_amount = df.groupby(by=['Garden']).count()
# print(house_amount)

# 08 小区房屋特征分析
# 查询房屋平均单价最便宜和最贵的小区名
# house_n = df.groupby('Garden')['PerPrice'].mean().sort_values(ascending=False).to_frame().reset_index()
# print(house_n)
# 查询房屋平均年份最老和最新的区县名
# house_x = df.groupby('Region')['Year'].mean().sort_values(ascending=False).to_frame()
# print(house_x)
...
house_mean_price = df.groupby(by=['Garden', 'Layout'])['Price'].mean()
print(house_mean_price)

house_mean_size = df.groupby(by=['Garden', 'Layout'])['Size'].mean()
print(house_mean_size)

house_mean_floor = df.groupby(by=['Garden', 'Layout'])['Floor'].mean()
print(house_mean_floor)

house_region = df['Region'].unique()
print(house_region)

# house_district = df['District'].unique()
